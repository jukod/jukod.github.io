NCPC @ JTH / JU
===============

Nordic Collegiate Programming Contest

<img src="https://github.com/user-attachments/assets/60c504fb-b859-4fe0-960b-13c9bb5bf50c" width=100% />

2026 Rudy Matela ([rudy.matela@ju.se]), JTH

[rudy.matela@ju.se]: mailto:rudy.matela@ju.se

------

What is the [NCPC][]?

[Nordic Collegiate Programming Contest][]

<img src="https://github.com/user-attachments/assets/60c504fb-b859-4fe0-960b-13c9bb5bf50c" width=100% />

We host it at JU every year.

[has been participating]: https://jukod.github.io/blog
[NCPC]: https://nordic.icpc.io/
[NCPC 2026]: https://nordic.icpc.io/
[Nordic Collegiate Programming Contest]:        https://nordic.icpc.io/
[Nordic Collegiate Programming Contest (NCPC)]: https://nordic.icpc.io/
[jukod.github.io]: https://jukod.github.io/

------

::: columns

:::: {.column width=50%}

The NCPC format:

* 👥 teams of 3 students
* ⏱️ 5 hours
* 🚩 [≈ 10 programming problems][]
* ⌨️ solutions in: Python, C, Java, C#, C++, Haskell or [40 other languages].
* 💻 1 computer per team
* 🎈 the team with [most problems solved][] wins

&nbsp;

* 1 problem solved = 1 balloon!

[≈ 10 programming problems]: https://github.com/icpc/ncpc-web/releases/download/ncpc2025-data/ncpc2025problems.pdf
[40 other languages]: https://open.kattis.com/languages
[most problems solved]: https://ncpc25.kattis.com/contests/ncpc25/standings/icpc

::::

:::: {.column width=50%}

<img src="https://github.com/user-attachments/assets/f582d523-0bfa-44c1-a87e-76ca7c1f2954" width=100% />

::::

:::

------

## NCPC 2026

__Where?__ &nbsp; Here at JU / JTH.

__When?__ &nbsp; Saturday 3 October.

<img src="https://github.com/user-attachments/assets/aae57475-676c-40e8-87e7-590356985fd8" width=100% />

------

::: columns

:::: {.column width=50%}
How are the problems?

* Programming
* Algorithms
* Data Structures
* From easy...
* ... to hard.


::::

:::: {.column width=50%}
<img src="https://live.staticflickr.com/1645/26110858644_84ba0f26c4_b.jpg" width=100% />
::::

:::

------

Example:

The
"[km/h](https://open.kattis.com/problems/kmh)"
problem
from 
[NCPC 2025](https://github.com/icpc/ncpc-web/releases/download/ncpc2025-data/ncpc2025problems.pdf)

------

* Solutions not only have to produce the correct output ✅,

* but also be in the optimal/expected time complexity ✅✅.

* It's often easy to get the correct output through brute-force or simulation (no points ❌),

* but not so easy to do it in an optimal time complexity ✅.

<img src="https://github.com/user-attachments/assets/bd662423-e424-4ffb-b483-6f92a35cfc51" width=100% />

------

### NCPC Schedule, Saturday, 3 October 2026

From 10:00 to 16:30

| Time           | Event                     | Location         |
| -------------- | ------------------------- | ---------------- |
| 10:00 -- 10:15 | Recommended arrival       | [JTH (yellow building)](#jth) |
| 10:15 -- 10:45 | Introduction Presentation | Stora Enso E1029 |
| 11:00 -- 16:00 | Main Contest (NCPC 2026)  | Husqvarna E1028  |
| Around 12:00   | Light lunch is served     | Galleriet E1120  |
| 16:00 -- 16:30 | Quick Award Ceremony      | Stora Enso E1029 |

<img src="https://github.com/user-attachments/assets/e99e464e-58ff-4e0e-b0b0-ce608b0c9937" width=100% />

------

::: columns

:::: {.column width=50%}

__Why participate?__

* Its __fun__!  You get baloons 🎈🎈🎈!
* A great way to __practice programming__: data structures, algorithms, etc.
* __Coursework__: gets __easier__.
* Teamwork.
* Meet people / networking.
* Potential travel (if you qualify)
* Nice for your CV.
* Meet companies (event sponsors).

::::

:::: {.column width=50%}

<img src="https://live.staticflickr.com/1526/26113291873_9208648a69_b.jpg" width=100% />

<img src="https://live.staticflickr.com/7884/32596056617_5dc85ee500_b.jpg" width=100% />

<img src="https://miro.medium.com/v2/resize:fit:720/format:webp/1*mWjv-mIg4gGw7vhj-uCyRw.jpeg" width=100% />

::::

:::


------

How to register in the NCPC?

1. form a team of up to 3 team members
2. __email <[rudy.matela@ju.se]>__ info for the (3) team members:
	- __full name__;
	- __university email address (e.g.: `@student.ju.se`)__;
	- __a team name__.
3. __wait for an email__ with a link to complete registration
4. __complete the registration__ on the received link

<img src="https://github.com/user-attachments/assets/6fc88168-9a91-4f0e-858c-d1db88798bda" width=100% />

------

::: columns

:::: {.column width=50%}

### Any questions?

### [rudy.matela@ju.se] 

### [jukod.github.io]

<img src="qr.svg" width=66% />

::::

:::: {.column width=50%}

<img src="https://github.com/user-attachments/assets/f582d523-0bfa-44c1-a87e-76ca7c1f2954" width=100% />

::::

:::

## [jukod.github.io/slides](https://jukod.github.io/slides)


------

.

------

### Preparing/practicing for the NCPC

The best problems for practicing leading up to the competition
are the
[problems from NCPC 2025](https://open.kattis.com/problem-sources/Nordic%20Collegiate%20Programming%20Contest%20%28NCPC%29%202025?order=difficulty_data),
[NCPC 2024](https://open.kattis.com/problem-sources/Nordic%20Collegiate%20Programming%20Contest%20%28NCPC%29%202024?order=difficulty_data) and
[NCPC 2023](https://open.kattis.com/problem-sources/Nordic%20Collegiate%20Programming%20Contest%20%28NCPC%29%202023?order=difficulty_data)
(last years).
Here are 6 interesting problems in order of difficulty:

- [F -             FizzBuzz &nbsp; 🐝](https://open.kattis.com/problems/fizzbuzz)
- [C -     Coffee Cup Combo &nbsp; ☕](https://open.kattis.com/problems/coffeecupcombo)
- [K -       Knot Knowledge &nbsp; 🪢](https://open.kattis.com/problems/knotknowledge)
- [L -         Locust Locus &nbsp; 🦗](https://open.kattis.com/problems/locustlocus)
- [H -         Highest Hill &nbsp; ⛰️](https://open.kattis.com/problems/highesthill)
- [J -             Jamboree &nbsp; ⛺](https://open.kattis.com/problems/jamboree)

They are [available for practice at open.kattis.com](https://open.kattis.com/problem-sources/Nordic%20Collegiate%20Programming%20Contest%20%28NCPC%29%202022?order=difficulty_data).
You can submit your solutions online there, so you can check if they are really correct.

------

::: columns

:::: {.column width=33%}

The competition stages:

1. [NCPC][]: Nordics
2. [NWERC][]: NW EU
3. [EUC][]: Europe
4. [ICPC][]: World Finals

[NWERC]: https://nwerc.eu/
[EUC]:   https://euc.icpc.global/
[ICPC]:  https://icpc.global/

Qualifying teams move to the next stage.

::::

:::: {.column width=66%}
<img src="https://jukod.github.io/ncpc-icpc.svg" width=100% />
::::

:::

------

There's an intricate web of competitions worldwide...

<img src="https://jukod.github.io/full-icpc.svg" width=100% />

------

<img src="https://jukod.github.io/ncpc-icpc.svg" width=100% />

Start here! &nbsp; ⬆️ 


------

### JU/JTH @ NCPC: summary

| NCPC              | #-solved&nbsp;ps. | 🇸🇪-rank     | 🇸🇪%   | rank            | %           |
| ----------------- | ----------------- | ------------- | ------- | --------------- | ----------- |
| [NCPC 2023][23sb] | 5 🎈🎈🎈🎈🎈      |    24th / 36  | top 66% |  89th / 160     | top 55%     |
| [NCPC 2024][24sb] | 4 🎈🎈🎈🎈        | __9th__ / 23  | top 40% |  __39th__ / 139 | __top 30%__ |
| [NCPC 2025][25sb] | 5 🎈🎈🎈🎈🎈      | __13th__ / 49 | top 27% |  __41th__ / 173 | __top 25%__ |

... also: [3 problems solved at NWERC 2024][]!

<img src="https://github.com/user-attachments/assets/b719083b-b6af-4074-85ce-fdf3c9a49633" width=55% />
<img src="https://github.com/user-attachments/assets/f582d523-0bfa-44c1-a87e-76ca7c1f2954" width=30% />

[23sb]: https://ncpc23.kattis.com/contests/ncpc23/standings?filter=5254
[24sb]: https://ncpc24.kattis.com/contests/ncpc24/standings?filter=6262
[25sb]: https://ncpc25.kattis.com/contests/ncpc25/standings/icpc
[3 problems solved at NWERC 2024]: https://2024.nwerc.eu/main/scoreboard/

------

### JU/JTH @ NCPC: #-teams summary

| NCPC              | #-teams                         | #-students                                                              |
| ----------------- | ------------------------------- | ----------------------------------------------------------------------- |
| [NCPC 2023][23sb] | 👥 (1)                          | 🎓🎓🎓 (3)                                                              |
| [NCPC 2024][24sb] | 👥👥👥👥 (4)                    | 🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓 (11)                                             |
| [NCPC 2025][25sb] | 👥👥👥👥👥👥👥👥👥👥 (10)       | 🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓 (30)       |
| &nbsp;            |                                 |                                                                         |
| __NCPC 2026__     | 👥👥👥👥👥👥👥👥👥👥... (¿≥10?) | 🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓🎓... (¿≥30?) |

&nbsp;

<img src="https://github.com/user-attachments/assets/2050e373-e9d2-4081-9b1c-667df7ae22ba" width=100% />



------

.

------

## Gallery

------

<img src="https://github.com/user-attachments/assets/0138649e-1ae2-4e77-b6f3-29c6b4d23c43" width=100% />

[NCPC 2025](https://nordic.icpc.io/ncpc2025/) @ JTH/JU.

------

<img src="https://github.com/user-attachments/assets/b719083b-b6af-4074-85ce-fdf3c9a49633" width=66% />

[NWERC 2024](https://2024.nwerc.eu/) @ Delft, Netherlands: time lapse #1.

------

::: columns

:::: {.column width=50%}

<img src="https://github.com/user-attachments/assets/f582d523-0bfa-44c1-a87e-76ca7c1f2954" width=100% />

::::

:::: {.column width=50%}

[NWERC 2024](https://2024.nwerc.eu/) @ Delft, Netherlands: time lapse #2.

::::

:::

------

::: columns

:::: {.column width=55%}
<img src="https://live.staticflickr.com/1645/26110858644_84ba0f26c4_b.jpg" width=100% />
::::

:::: {.column width=45%}

ICPC 2009 World Finals @ Stockholm, Sweden

::::

:::

------

::: columns

:::: {.column width=50%}

* ICPC 2009 World Finals, Stockholm
* ICPC 2019 World Finals, Porto
* ICPC 20?? World Finals, ????

::::

:::: {.column width=50%}

<img src="https://live.staticflickr.com/1526/26113291873_9208648a69_b.jpg" width=100% />

<img src="https://live.staticflickr.com/7884/32596056617_5dc85ee500_b.jpg" width=100% />

<img src="https://miro.medium.com/v2/resize:fit:720/format:webp/1*mWjv-mIg4gGw7vhj-uCyRw.jpeg" width=100% />

::::

:::

------

.

------

To compile this document from source run:

```bash
pandoc --title "JU Kod" -tslidy -s slides.md -o slides.html
```

You need `pandoc` installed.

This slide set can be found on:

```
https://jukod.github.io/slides
```

------

.
